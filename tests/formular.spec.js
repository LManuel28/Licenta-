...
// 2. STRUCTURA PAGINII
// ──────────────────────────────────────────────
test('formularul și heading-ul principal există pe pagină', async ({ page }) => {
  await page.goto(BASE_URL);
...
// ──────────────────────────────────────────────
test('selectorul de tip eveniment este primul câmp vizibil', async ({ page }) => {
  await page.goto(BASE_URL);
  await page.waitForTimeout(200);
  await expect(page.locator('#eventType')).toBeVisible();
});

test('câmpurile apar progresiv după completarea fiecăruia', async ({ page }) => {
  await page.goto(BASE_URL);
  await page.waitForTimeout(200);
...
// ──────────────────────────────────────────────
test('câmpul telefon respinge literele și acceptă doar cifre', async ({ page }) => {
  await page.goto(BASE_URL);
  await page.waitForTimeout(200);
  await page.selectOption('#eventType', 'Majorat');
  await page.waitForTimeout(200);
...
test('câmpul dată are atributul min setat (nu permite date în trecut)', async ({ page }) => {
  await completeStep1(page);
  await page.locator('#btnNext').click();
  await expect(page.locator('#eventDate')).toBeVisible({ timeout: 3000 });
...
test('butonul Next nu este vizibil la încărcarea paginii', async ({ page }) => {
  await page.goto(BASE_URL);
  await page.waitForTimeout(300);
  const isHidden = await page.locator('#btnNext').evaluate(el => el.classList.contains('btn-hidden'));
  expect(isHidden).toBeTruthy();
});

test('butonul Next apare doar după completarea tuturor câmpurilor din pasul 1', async ({ page }) => {
  await page.goto(BASE_URL);
  await page.waitForTimeout(200);
...
// ──────────────────────────────────────────────
test('formularul pentru Majorat poate fi completat corect până la pasul 3', async ({ page }) => {
...
// ──────────────────────────────────────────────
test('selectarea Nuntă afișează câmpuri specifice nuntă în pasul 2', async ({ page }) => {
  await completeStep1(page, 'Nuntă');
  await page.locator('#btnNext').click();
  await expect(page.locator('#panel-2')).toBeVisible({ timeout: 3000 });
  await page.waitForTimeout(300);

  await expect(page.locator('input[name="Zona mireasă"]')).toBeVisible({ timeout: 5000 });
  await expect(page.locator('input[name="Zona mire"]')).toBeVisible({ timeout: 5000 });
});

test('selectarea Corporate nu afișează câmpurile de nuntă', async ({ page }) => {
...
test('pasul 3 afișează panoul corespunzător tipului de eveniment', async ({ page }) => {
  await completeStep1(page, 'Corporate');
  const nextBtn = page.locator('#btnNext');
  await expect(nextBtn).toBeVisible({ timeout: 5000 });
  await nextBtn.click();
  ...
  await expect(page.locator('#panel-4-corporate')).toBeVisible({ timeout: 5000 });
...
// 9. STEPPER
// ──────────────────────────────────────────────
test('stepper-ul există și pasul 1 este activ la încărcare', async ({ page }) => {
  await page.goto(BASE_URL);
  await expect(page.locator('#stepper')).toBeVisible();
  await expect(page.locator('#dot-1')).toHaveClass(/active/);
});

test('stepper-ul se actualizează după trecerea la pasul 2', async ({ page }) => {
